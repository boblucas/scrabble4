...
while read -r ub mask; do
  ...
  or_ok=$(grep -l "VERDICT: CERTIFIED" experiments/results/english/oxy_or_${tag}_lb${LB}*.log experiments/results/english/oxy_or_${tag}_lb178*.log 2>/dev/null | head -1)
  tb_need=1; [ "$LB" -ge $((ub-27)) ] && tb_need=0
  tb_ok=$(grep -l "CERTIFIED" experiments/results/english/oxy_tb_${tag}_lb${LB}*.log experiments/results/english/oxy_tb_${tag}_lb178*.log 2>/dev/null | head -1)
