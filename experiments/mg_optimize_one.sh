...
for it in $(seq 1 "$NITER"); do
  [ -f experiments/results/mg_OPTBEAT ] && break
  sb=$(( SEEDOFF + (it-1)*3 ))
  ...
  if [ -f experiments/results/mg_decomp_OPT_${TAG}.json ]; then
    ...
    if [ "$okv" = "ok=True" ] && [ -n "$sc" ]; then
      ...
      if [ "$sc" -gt "$RECORD" ]; then
        echo ">>> RECORD VERSLAGEN: ${sc} > ${RECORD} (${R0}/${R7}/${R14}) <<<" >> "$LOG"
        cp experiments/results/mg_gamemax_OPT_${TAG}.json experiments/results/mg_OPTBEAT_${TAG}_${sc}.json
        touch experiments/results/mg_OPTBEAT
        break
      fi
    fi
