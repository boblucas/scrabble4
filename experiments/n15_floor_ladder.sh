...
# Usage: n15_floor_ladder.sh <mask> [word] [lexicon] [dict] [floors...]
set -u
cd /home/bob/programming/scrabble4
MASK="$1"
WORD="${2:-geschenkcheques}"
LEX="${3:-dutch2026}"
XD="${4:-experiments/xtests/dict_2026_15.txt}"
shift; shift 2>/dev/null; shift 2>/dev/null; shift 2>/dev/null
FLOORS="${*:-425 415 405 395 385 376}"
TAG=$(echo "$MASK" | tr ',' '_')
DTAG=$(echo "$MASK" | tr -d ',')
if [ "$WORD" = geschenkcheques ]; then BASE=experiments/results/oracle_parallel/bases_2026/geschenkcheques/base_${TAG}.txt; else BASE=experiments/results/oracle_parallel/ctc/base_${TAG}.txt; fi
D=experiments/results/oracle_parallel/ladder_${WORD}_${DTAG}
mkdir -p "$D"
...
for FLOOR in $FLOORS; do
  ...
  "$BIN" --pinenum "$BASE" --floor "$FLOOR" --mainletters "$ML" --shards ${NSHARD:-20} --outdir "$SD" 2>/dev/null | tee "$SD/pinenum.log"
  ...
  for i in $(seq 0 $(( ${NSHARD:-20} - 1 ))); do
    PINWALL=5 "$BIN" --pinbatch "$BASE" --emit < "$SD/shard_$(printf %02d $i).txt" > "$SD/verdict_$(printf %02d $i).txt" 2>/dev/null &
  done
  wait
  ...
  if [ "${NM:-0}" -gt 0 ]; then
    ...
    N15_LANG=$LEX N15_HMAX=15 .venv/bin/python -u experiments/_ladder_witness.py "$MASK" "$SD" "${LADDER_LB:-2100}" "$WORD" && { echo "[$MASK] NEW LB gevonden -- ladder stopt"; exit 0; }
  fi
