...
def run_one(R0, R7, R14, plan):
    ...
    if os.environ.get('MGQPASS'):
        qwords = [w for w in words if 'q' in w and 2 <= len(w) <= 8]
        qc = []
        anchors_q = [(x, y) for y in range(15) for x in range(15) if grid[y][x]]
        for w in qwords:
            for (ax, ay) in anchors_q:
                ...
                for i, ch in enumerate(w):
                    ...
                    for (px, py, h) in ((ax-i, ay, 1), (ax, ay-i, 0)):
                        ...
                        if e > 0: qc.append((e, w, px, py, h))
        qc.sort(key=lambda t: -t[0])
        for (e, w, px, py, h) in qc[:400]:
            if play(w, px, py, h):
                print(f"Q-PASS: {w} ({px},{py}){'h' if h else 'v'} est~{e}", flush=True)
                break
    for rnd in range(40):
