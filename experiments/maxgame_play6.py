...
print(f"# {len(triples)} triples", flush=True)
def run_one(R0, R7, R14, plan):
    global grid, moves, used, blankcells, blanks_left, MASKC
    C7 = R7[4:11]
    ...
    def play(word, x, y, h, final=False):
        ...
        if okr and not final:
            # sluitend: elke lege maskercel gesimuleerd -> eindkolomrun moet woord zijn
            for (mx, my) in MASKC:
                ...
                if up or dn:
                    L = (R0 if my == 0 else R7 if my == 7 else R14)[mx]
                    if not isw(''.join(reversed(up)) + L + ''.join(dn)): okr = False; break
        if not okr:
    ...
    def snap():
        return ([row[:] for row in grid], [mv[:] for mv in moves], Counter(used),
                set(blankcells), blanks_left)
    def restore(st):
        global blanks_left
        g2, m2, u2, b2, bl2 = st
        for y in range(15): grid[y][:] = g2[y]
        moves[:] = [mv[:] for mv in m2]
        used.clear(); used.update(u2)
        blankcells.clear(); blankcells.update(b2)
        blanks_left = bl2
    def gen_cands(topa=35, topw=1800, keep=200):
        anchors = [(x, y) for y in range(15) for x in range(15) if grid[y][x]]
        random.shuffle(anchors)
        out = []
        for (ax, ay) in anchors[:topa]:
            achr = chr(96+grid[ay][ax])
            for w in let2w.get(achr, [])[:topw]:
                for i, ch in enumerate(w):
                    if ch != achr: continue
                    for (px, py, h) in ((ax-i, ay, 1), (ax, ay-i, 0)):
                        e = est_score(w, px, py, h)
                        if e > 0: out.append((e, w, px, py, h))
        out.sort(key=lambda t: (-t[0], -len(t[1])))
        return out[:keep]
    BEAM = int(os.environ.get('MGBEAM', '6')); BRANCH = int(os.environ.get('MGBRANCH', '4'))
    beam = [(0, snap())]
    for rnd in range(28):
        nxt = []; any_move = False
        for cum, st in beam:
            ...
            for (e, w, px, py, h) in cands:
                if tried >= BRANCH: break
                restore(st)
                if play(w, px, py, h):
                    nxt.append((cum+e, snap())); tried += 1; any_move = True
            if tried == 0:
                restore(st)
                done = False
                anchors = [(x, y) for y in range(15) for x in range(15) if grid[y][x]]
                ...
                for (ax, ay) in anchors[:70]:
                    ...
                    for w in let2w.get(achr, []):
                        ...
                        if done: break
                    if done: break
                if done: nxt.append((cum+1, snap())); any_move = True
                else: nxt.append((cum, st))
        seen = set(); uniq = []
        for cum, st in sorted(nxt, key=lambda t: -t[0]):
            key = tuple(tuple(row) for row in st[0])
            if key in seen: continue
            seen.add(key); uniq.append((cum, st))
        beam = uniq[:BEAM]
    ...
    for cum, st in beam:
        ...
        if bestev is None or (ok2 and (not bestev[1] or tot > bestev[0])):
            bestev = (tot, ok2, msg, s7, s0, s14, snap())
    tot, ok2, msg, s7, s0, s14, stbest = bestev
    restore(stbest)
    ...
    out = {'grid': [row[:] for row in grid], 'moves': [[list(c) for c in mv] for mv in moves],
...
for k, (sc, w0, w7, w14) in enumerate(triples[:6000]):
    ...
    if p:
        found.append((sc, w0, w7, w14, p))
        print(f"planbaar #{len(found)}: triple[{k}] backbone={sc} r0={w0} r7={w7} r14={w14}", flush=True)
        if len(found) >= NTRY: break
if not found: print("GEEN ENKEL PLAN"); sys.exit(1)
best = None
for (sc, w0, w7, w14, p) in found:
    tot, ok2, out = run_one(w0, w7, w14, p)
    print(f"UITKOMST backbone={sc}: score={tot} ok={ok2}", flush=True)
    if ok2 and (best is None or tot > best[0]): best = (tot, out)
if best is None: print("geen geldig spel"); sys.exit(1)
tot, out = best
print(f"BESTE VAN {len(found)}: {tot}", flush=True)
json.dump(out, open(f'experiments/results/maxgame_play6_{os.environ.get("SEED","1")}.json', 'w'))
