...
# Dictionary as a set of letter-strings, for SETUP tail-legality.  A scored vertical w = main[c]+tail
# hangs the tail (rows 1..len(tail)) as a STANDALONE maximal run in the pre-turn (setup) board; if
# len(tail) >= 2 that tail must ITSELF be a legal dictionary word for the position to be reachable.
_WORDSET = set(r.words_str)


def _tail_setup_legal(word):
    """True iff the vertical `word` (= top + tail) is SETUP-legal: its tail (word[1:]) is allowed to
    stand alone in the pre-turn board.  A length-<=1 tail is a single tile (no run) -> always ok;
    a length->=2 tail must itself be a dictionary word."""
    tail = word[1:]
    return len(tail) <= 1 or tail in _WORDSET


# best single-column SETUP-LEGAL tail value over dict words "L+tail", len 2..8, per first letter
_TAILMAX = {}
for word in r.words_str:
    if 2 <= len(word) <= HMAX and _tail_setup_legal(word):
...
def vert_table(top_letter, Ltail):
    ...
    seen = {tuple([0] * Ltail)}
    for word in r.words_str:
        if 2 <= len(word) <= Ltail + 1 and word[0] == top_letter and _tail_setup_legal(word):
