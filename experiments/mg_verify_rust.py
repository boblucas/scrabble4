...
R0=os.environ.get('VR0','geschenkcheques');R7=os.environ.get('VR7','verzwaringswerk');R14=os.environ.get('VR14','polymelkzuurtje')
...
moves=prep+finals; blanks=set(pcells(blenc))
tot,per,ok,msg=MG.score_game([r[:] for r in grid],moves,blanks)
print(f"RUST-VERIFY: rust={total} python={int(tot)} ok={ok} match={int(tot)==total} ({msg}) zetten={len(moves)}")
if ok and int(tot)>=int(os.environ.get('MINPROMOTE','999999')):
    D={'grid':grid,'moves':[[list(c) for c in mv] for mv in moves],'blanks':[list(b) for b in blanks],
       'total':int(tot),'triple':[R0,R7,R14],'plan':f'reachable record {int(tot)} via Rust pipeline, Python-geverifieerd'}
