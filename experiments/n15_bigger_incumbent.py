...
import n15_bigger_v2 as V                # bigger-lexicon full-turn model (verticals on ALL cols)
...
def verify(word, mask, grid):
    ...
    r2 = V.construct_rules('dutch', B, word_file=WORD_FILE)
...
def save(word, mask, grid, total):
    ...
    blob = {'board': B, 'main_word': word, 'turn_str': turn, 'require_center': True,
            'claimed_total': total, 'lexicon': WORD_FILE, 'grid': grid}
    # distinct N15_bigger_best_* namespace so the concurrent dutch cert (which polices N15_best_*)
    # never touches our bigger-lexicon witnesses.
