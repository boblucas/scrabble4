...
R0='royaltywatchers';R7='pulsoxymetertje';R14='quichebuffetjes'
mv=[
 [(7,y) for y in range(4,11)],                                  # centrum-vert kol 7 ([3]='m')
 [(c,7) for c in (4,5,6,8,9,10,11)],                            # OXYMETER
 [(4,y) for y in (3,4,5,6,8,9,10)],                             # x4-vert kol 4 (door 'o')
 [(10,y) for y in (3,4,5,6,8,9,10)],                            # x4-vert kol 10 (door 'e')
 [(8,y) for y in range(7)],                                     # top-vert kol 8 (a..e, 1452!) incl (8,3)
 [(x,3) for x in (2,3,5,6,7,9,11)],                             # laan [2,11] (7 nieuw; x4 (3,3)+(11,3))
 [(4,11)],[(10,11)],                                            # exts -> 9-woorden
 [(3,0),(3,1),(3,2)],[(5,0),(5,1),(5,2)],                       # 3-stubs (laan-geworteld)
 [(2,0),(2,1),(2,2)],                                           # stub kol 2 (laan-geworteld)
 [(4,0)],                                                       # 'alt'-ketting
 [(9,0),(9,1),(9,2)],                                           # stub kol 9 (laan-geworteld)
 [(11,2)],                                                      # anker voor kol-12-vert
 [(12,y) for y in range(7)],                                    # top-vert kol 12 (e..t)
 [(11,0),(11,1)],                                               # ministub 11 (via (11,2))
 [(9,y) for y in range(8,15)],                                  # bottom-vert kol 9 (t..f)
 [(11,y) for y in range(8,15)],                                 # bottom-vert kol 11 (r..t)
 [(8,y) for y in range(8,15)],                                  # bottom-vert kol 8 (e..f, 17)
 [(c,7) for c in (0,1,2,3,12,13,14)],                           # fin7
 [(c,0) for c in (0,1,6,7,10,13,14)],                           # fin0 (pre0={2,3,4,5,8,9,11,12})
 [(c,14) for c in (0,3,7,10,12,13,14)],                         # fin14 (pre14={1,2,4,5,6,8,9,11})
]
ins=mv.index([(c,7) for c in (0,1,2,3,12,13,14)])
west=[[(7,13)],[(6,13)],[(5,13)],[(4,13)],[(3,13)],[(2,13)],[(1,13)],[(1,14)],[(2,14)],[(4,14)],[(5,14)],[(6,14)]]
# (7,13)? kol 7: gaskast rijen 4-10, (7,11),(7,12) leeg -> (7,13) zweeft. Wortel: (8,13)-bottom-vert-cel!
west=[[(7,13)]]+west[1:]
mv=mv[:ins]+west+mv[ins:]
